...
WebSocketsClient::WebSocketsClient() {
    ...
    _client.extraHeaders = WEBSOCKETS_STRING(""); // for Spring boot
...
void WebSocketsClient::begin(const char * host, uint16_t port, const char * url, const char * protocol) {
    ...
    DEBUG_WEBSOCKETS("[WS-Client] Websocket Version1: " WEBSOCKETS_VERSION "\n");
}
...
void WebSocketsClient::messageReceived(WSclient_t * client, WSopcode_t opcode, uint8_t * payload, size_t length, bool fin) {
    WStype_t type = WStype_ERROR;
     ...
     DEBUG_WEBSOCKETS("####[WS-Client] client messageReceived client.\n", client);
      DEBUG_WEBSOCKETS("####[WS-Client] client messageReceived opcode.\n", opcode);
       DEBUG_WEBSOCKETS("####[WS-Client] client messageReceived payload.\n", payload);
...
bool WebSocketsClient::clientIsConnected(WSclient_t * client) {
    ...
    if(client->tcp->connected()) {
        ...
        // client lost
        if(client->status != WSC_NOT_CONNECTED) {
            DEBUG_WEBSOCKETS("[WS-Client] connection lost.  aaa \n" + client->status);
            // do cleanup
...
void WebSocketsClient::handleClientData(void) {
    if((_client.status == WSC_HEADER || _client.status == WSC_BODY) && _lastHeaderSent + 300000 < millis()) {
...
void WebSocketsClient::sendHeader(WSclient_t * client) {
    ...
    DEBUG_WEBSOCKETS("[WS-Client][sendHeader] client->cKey\n", client->cKey);
        ...
        if(headerLine->startsWith(WEBSOCKETS_STRING("HTTP/1."))) {
            ...
            if(headerName.equalsIgnoreCase(WEBSOCKETS_STRING("Connection"))) {
                ...
                if(headerValue.indexOf(';') > -1) {
                    ...
                    if (headerValue.indexOf("HttpOnly") > -1) { client->cSessionId = headerValue.substring(headerValue.indexOf('=') + 1, headerValue.indexOf(";")); } else { client->cSessionId = headerValue.substring(headerValue.indexOf('=') + 1); }
                }
            }
        } else {
